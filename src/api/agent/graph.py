...
from api.agent.agents import Delegation, RAGUsedContext, ToolCall, coordinator_agent, product_qa_agent, shopping_cart_agent
...
workflow.add_conditional_edges(
    "coordinator_agent",
    coordinator_router,
    {"product_qa_agent": "product_qa_agent", "shopping_cart_agent": "shopping_cart_agent", "end": END},
)

workflow.add_conditional_edges(
    "product_qa_agent", product_qa_agent_tool_router, {"tools": "product_qa_agent_tool_node", "end": "coordinator_agent"}
)

workflow.add_conditional_edges(
    "shopping_cart_agent", shopping_cart_agent_tool_router, {"tools": "shopping_cart_agent_tool_node", "end": "coordinator_agent"}
)
    ...
    def _process_graph_event(chunk):
        ...
        def _tool_to_text(tool_call):
            if tool_call.name == "get_formatted_item_context":
                ...
                return f"Adding items to cart..."
            elif tool_call.name == "get_shopping_cart":
                return f"Retrieving your cart..."
            elif tool_call.name == "remove_from_cart":
                return f"Removing item from cart..."
            else:
                return "Processing..."
        ...
        if _is_node_start(chunk):
            ...
            if node_name == "coordinator_agent":
                ...
                return "Managing your shopping cart..."
            elif node_name == "product_qa_agent_tool_node":
                # Tool node executes based on tool_calls from messages
